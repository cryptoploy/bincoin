...
uint256 hashGenesisBlock("0x344f2b0975709fda8efa9725d92fe2affe7b13d555038b2a13335855a5cd3e3f");
static CBigNum bnProofOfWorkLimit = CBigNum().SetCompact(504365644); // Bincoin: starting difficulty is 1 / 2^12
...
{
    // Bincoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 3500000000;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 300000); // Bincoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 302400; // Bincoin: 3.5 days
static const int64 nTargetSpacing = 150; // Bincoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bincoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bincoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Bincoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x15a6eeb03a5db12c2a73d329ba292d6cd251e0f9f8c1fa4e1d1862f620409bdf");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BNC +";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 3500000000;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("be0552e4cba3c1689357aade4851bd3e811b4e270076524ddc5d96c6023261dca3c98bfb7fd09c7fbaee80cd628d4c7988f50bb6986ffb22773d1c0989fc9c49d2") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1507270270;
        block.nBits    = 504365644;
        block.nNonce   = 850793058;

        if (fTestNet)
        {
            block.nTime    = 1507270270;
            block.nNonce   = 3275085570;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x80990cd16c88647c80ed950bda31aaef9b7531ee7f50f240c2bad91e4b7fde62"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Bincoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BincoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BincoinMiner : ProcessBlock, block not accepted");
    }
